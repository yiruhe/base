...
public class ShowStudentTable extends HttpServlet{
		...
		throws IOException,ServletException
		{
			...
			QueryParameter queryObj = new QueryParameter();
		
			//给对象添加请求中的参数
			this.insertinserStudentData(request,queryObj);
			
			//用于数据回现
			request.setAttribute("dir",queryObj);
			
			//获取系种类
			List dirList = PartNameDao.query();
			
			request.setAttribute("partName",dirList);
			
			//查询
			PageList pageList = studentDao.PageQuery(queryObj);
		...
		protected void insertinserStudentData(HttpServletRequest request,QueryParameter queryParameter){
			...
			if(currentPage != null && !"".equals(currentPage)){
				...
				queryParameter.setCurrentPage(Integer.valueOf(currentPage));
			}
			
			if(pageSize != null && !"".equals(pageSize)){
				
				queryParameter.setPageSize(Integer.valueOf(pageSize));
